...
task runr, "Runs gameoflife for current platform":
 exec &"nim c -r {releaseOpts} -o:gameoflife src/main.nim"

task rund, "Runs debug gameoflife for current platform":
 exec &"nim c -r {debugOpts} -o:gameoflife src/main.nim"

task release, "Builds gameoflife for current platform":
 exec &"nim c {releaseOpts} -o:gameoflife src/main.nim"

task webd, "Builds debug gameoflife for web":
 exec &"nim c {debugOpts} -d:emscripten -o:gameoflife.html src/main.nim"

task webr, "Builds release gameoflife for web":
 exec &"nim c {releaseOpts} -d:emscripten -o:gameoflife.html src/main.nim"

task debug, "Builds debug gameoflife for current platform":
 exec &"nim c {debugOpts} -o:gameoflife_debug src/main.nim"
